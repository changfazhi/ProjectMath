...
25. `031_user_stripe_subscription.sql` — adds `stripe_subscription_id TEXT` to `users` so a live card subscription is a fact on the row, independent of `access_expires_at` (PayNow). Backfills pre-existing card subscribers (`subscription_status = 'active' AND access_expires_at IS NULL AND stripe_customer_id IS NOT NULL`) with the sentinel `'legacy_unbackfilled'` — **without this they would all resolve to `free` on deploy**; the next `customer.subscription.updated` webhook replaces it with the real id. Fixes issue #57 (see [Authentication & Tiers](#authentication--tiers)).
...
- **Tiers:** `'free' | 'paid'`, derived **from the `users` row, never from the token's custom claim** — `deriveTier()` in `middleware/auth.ts`. The two products are **independent facts on the row, and either alone buys paid access**: `access_expires_at` in the future (PayNow paid-until), or `stripe_subscription_id` set with `subscription_status = 'active'` (a live card subscription). A card subscription therefore stays paid past a lapsed `access_expires_at` left behind by an earlier PayNow purchase — which is exactly the date its `trial_end` deferred to — and PayNow time survives that subscription being cancelled (issue #57). A Firebase ID token lives up to an hour and `setCustomUserClaims` cannot invalidate the tokens already issued, so a claim-derived tier let a cancelled card subscriber keep paid quotas for another hour (issue #54). The row is written synchronously by the Stripe webhook, so every grant and revocation binds on the user's next request; the read is free because `requireAuth` already fetches the row to resolve the internal user id. The claim survives only as a UI hint (see below). `gate(feature)` = `requireAuth` + a check against `config/featureTiers.ts` (currently every feature is tier-accessible to `'free'`; paid buys higher **usage quotas**, not feature access).
...
- **PayNow stacking & card/PayNow switching:** Checkout is never blocked by an existing active plan (the old "you already have an active subscription" guard was removed) — the *only* remaining block is a genuine card→card duplicate (`findActiveSubscription()` in `billingService.ts` checks for an existing active/trialing Stripe subscription). Buying **PayNow while PayNow time remains** stacks the new period on top of `access_expires_at` instead of restarting from today (computed in the `checkout.session.completed` webhook handler, mode `'payment'`). Buying **PayNow while a card subscription is active** rolls the card subscription over: it's scheduled `cancel_at_period_end: true` and the new PayNow period starts from that subscription item's `current_period_end` (not from today), so there's no coverage gap or double-billing. Because a card subscription bought over PayNow *trials until the PayNow expiry*, the rollover is **not** an `else` on the "PayNow time remains" branch — both are true at once, and skipping it would leave the card to start charging at `trial_end` on top of the period just bought. Buying **card while PayNow time remains** sets `subscription_data.trial_end` to the current `access_expires_at`, so the card isn't charged until the PayNow period actually runs out (Stripe grants `tier: paid` immediately on `checkout.session.completed` regardless, since the user was already paid via PayNow). **`grantPaidTier()` must never touch `access_expires_at`** — it used to null it, erasing PayNow time the user had already paid for, so cancelling the subscription during the trial walked straight past the guard below and dropped them to free (issue #57). `revokePaidTier()` (used by the cancellation/past_due/unpaid webhooks) always clears `stripe_subscription_id` — the subscription is gone either way — then checks `access_expires_at` before downgrading: if PayNow access is still valid, `subscription_status` and the claim are left alone and the row expires on its own.
- **`customer.subscription.updated`** does two jobs: `canceled`/`past_due`/`unpaid` → `revokePaidTier()`; `active`/`trialing` → re-assert `{ subscription_status: 'active', stripe_subscription_id: sub.id }`. The second is what keeps a card subscriber paid after the trial converts, and it also backfills migration 031's `'legacy_unbackfilled'` sentinel (every live subscription emits this at least once a billing cycle).
...
| Cancelling a card subscription wiped PayNow time the user still had | `grantPaidTier()` nulled `access_expires_at` when the subscription was bought, so `revokePaidTier()`'s guard read "no expiry". The two must stay independent — PayNow lives in `access_expires_at`, the card in `stripe_subscription_id` (migration 031, issue #57) |
| Every card subscriber dropped to `free` right after a deploy | Migration 031 wasn't run, so `stripe_subscription_id` is null for everyone and `deriveTier()` sees no live subscription. Run it — its backfill is what carries existing subscribers across |
| A card subscriber went `free` on the day their old PayNow date passed | `customer.subscription.updated` isn't reaching the webhook for `active`/`trialing`, so `stripe_subscription_id` was never recorded and the lapsed `access_expires_at` is all `deriveTier()` can see. Check the endpoint's subscribed events |
