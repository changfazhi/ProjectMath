...
- **Per-user cooldowns:** `services/cooldownService.ts` — in-memory, throttles *accepted* AI requests per user (`AI_CHAT_COOLDOWN_S`=5s, `AI_GRADE_COOLDOWN_S`=60s), separate from the daily quota and from the IP-keyed `express-rate-limit` on each route. An AI call that failed **on our side** (gateway outage, DB error) clears the stamp so the user doesn't lose their cooldown to it — but a `GradingError` (the model rejecting a junk photo) does **not** refund: that call really hit Gemini, and it writes no `gradings` row so it costs no quota either. Refunding it made junk submissions an unmetered vision call every few seconds (issue #56).
...
| POST | `/api/grade/text` | JSON `{ question_id, transcription_latex, time_taken_s? }` → re-grade the student's corrected LaTeX transcription (no photos, same limits as `/api/grade`; the 60s cooldown is waived only for the first correction of a recent photo scan — see [Photo-Based AI Grading](#photo-based-ai-grading)) |
