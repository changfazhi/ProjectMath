...
VALUES (
  ...
  $$$$,
  NULL,
  $$(a) The right-hand sum runs to \(r=n\), which is one term more than the left-hand sum (up to \(r=n-1\)), so the extra term \(\ln(n+1)\) must be subtracted: \(\displaystyle\sum_{r=1}^{n-1}\ln(r+1)=\sum_{r=1}^{n}\ln(r+1)-\ln(n+1)\).

(b) The right-hand sum starts at \(n=6\), so it is missing the terms for \(n=3,4,5\); these must be added back: \(\displaystyle\sum_{n=3}^{N-2}e^{n-1}=\sum_{n=6}^{N-2}e^{n-1}+\sum_{n=3}^{5}e^{n-1}\).$$,
  4,
...
VALUES (
  ...
  $$See parts$$,
  NULL,
  $$(i) \(\displaystyle\sum_{k=0}^{n}(k+n+1)=\sum_{k=0}^{n}k+(n+1)(n+1)=\dfrac{n}{2}(n+1)+(n+1)^2=(n+1)\left(\dfrac{3}{2}n+1\right)\).

(ii) \(\displaystyle\sum_{r=1}^{N}\left(4r+2^{-r}\right)=4\cdot\dfrac{N}{2}(N+1)+\dfrac{2^{-1}\left(1-(2^{-1})^{N}\right)}{1-2^{-1}}=2N(N+1)+1-\dfrac{1}{2^{N}}\).

(iii) \(\displaystyle\sum_{k=1}^{30}\left(a^{2k}+ka\right)=\dfrac{a^{2}\left(1-a^{60}\right)}{1-a^{2}}+a\cdot\dfrac{30}{2}(31)=\dfrac{a^{2}\left(a^{60}-1\right)}{a^{2}-1}+465a\).$$,
  6,
  'H2 Math Tutorial (Sigma Notation)',
  $$[
  {
